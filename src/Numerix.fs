...
let dataIsValid (list: double[]) =
    ...
    | list when containsInfinity list -> Error("Infinity is not allowed")
    | list when Array.exists Double.IsNaN list -> Error("NaN is not allowed")
    | _ -> Ok(list)
